---
title: "FUBO_Bayesian_single_parameter_complete"
author: "Peihong Qin"
date: "2025-07-14"
...
## FUBO (Full Borrowing) Bayesian method - Single Parameter
### Compatible situation

```{r}
# Set Nimble model
# Full Borrowing - Bayesian approach
...
code_fubo <- nimbleCode({
  ...
  for (i in 1:n2){
    y_full[i] ~ dnorm(beta0 + beta1_c * x_full[i], sd=sigma2)
  }
})
```
...
for (i in 1:n_sim) {
  ...
  # Save the results
  results_fubo_bayes_com[i, ] <- c(beta1c_hat, lower_CI, upper_CI)
...
# Basic statistics
bias_com <- mean(results_fubo_bayes_com$Estimate) - beta1_c
relative_bias_com <- bias_com / beta1_c
variance_com <- var(results_fubo_bayes_com$Estimate)
mean_mcmc_var_com <- mean(beta1c_variances_com)
mse_com <- mean((results_fubo_bayes_com$Estimate - beta1_c)^2)
rmse_com <- sqrt(mse_com)
ci_width_com <- mean(results_fubo_bayes_com$Upper_CI - results_fubo_bayes_com$Lower_CI)
coverage_prob_com <- mean(results_fubo_bayes_com$Lower_CI <= beta1_c & 
                         results_fubo_bayes_com$Upper_CI >= beta1_c)

# Output compatible scenario results
cat("=== FUBO Bayesian - Compatible Scenario ===\n")
...
beta0_incom <- 1
beta1_h_incom <- 2  # Different from beta1_c
beta1_c_incom <- 1
n <- 200 
n_sim <- 100
...
for (i in 1:n_sim) {
  ...
  # Save the results
  results_fubo_bayes_incom[i, ] <- c(beta1c_hat, lower_CI, upper_CI)
...
# Basic statistics
bias_incom <- mean(results_fubo_bayes_incom$Estimate) - beta1_c_incom
relative_bias_incom <- bias_incom / beta1_c_incom
variance_incom <- var(results_fubo_bayes_incom$Estimate)
mean_mcmc_var_incom <- mean(beta1c_variances_incom)
mse_incom <- mean((results_fubo_bayes_incom$Estimate - beta1_c_incom)^2)
rmse_incom <- sqrt(mse_incom)
ci_width_incom <- mean(results_fubo_bayes_incom$Upper_CI - results_fubo_bayes_incom$Lower_CI)
coverage_prob_incom <- mean(results_fubo_bayes_incom$Lower_CI <= beta1_c_incom & 
                           results_fubo_bayes_incom$Upper_CI >= beta1_c_incom)

# Output incompatible scenario results
cat("=== FUBO Bayesian - Incompatible Scenario ===\n")
